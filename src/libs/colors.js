...
function getRandom(minNumColors=3) {
  let filteredPalettes = getAll(minNumColors);
  return filteredPalettes[Math.floor(Math.random() * filteredPalettes.length)];
}
...
function getAll(minNumColors = 3) {
  return palettes.filter (palette => palette.colors.length >= minNumColors);
}

function getNames(minNumColors = 3) {
  let filteredPalettes =  palettes.filter(palette =>  palette.colors.length >= minNumColors)
  return filteredPalettes.map(p => p.name);
}
